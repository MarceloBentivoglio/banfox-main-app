class OpsAdmin::OperationsController < OpsAdmin::BaseController
  ...
  def analyse
    ...
    @operations = Operation.joins(:installments).where(installments: {backoffice_status: :ordered}).preload(:installments).distinct.paginate(page: params[:page])
  ...
  def deposit
    ...
    #TODO create a scope in operations
    @seller = current_user.seller
    @operations = Operation.joins(:installments).where(signed: true).where(installments: {backoffice_status: :approved}).preload(:installments).distinct.paginate(page: params[:page])
    respond_to do |format|
      format.html
      format.js
    end
