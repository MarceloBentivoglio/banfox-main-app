...
# No one can change the order of this array without reflecting the changes on the status invoice method
  ...
  scope :in_store, -> {joins(:installments).where(backoffice_status: [0,1]).group("invoices.id").having("SUM(CASE WHEN (liquidation_status = 0) THEN 1 ELSE 0 END) > 0").order("max(installments.due_date) DESC")}
  scope :overdue, -> {joins(:installments).where(backoffice_status: 2).group("invoices.id").having("SUM(CASE WHEN (liquidation_status = 0) AND (installments.due_date < NOW()) THEN 1 ELSE 0 END) > 0").order("max(installments.due_date) ASC")}
  scope :on_date, -> {joins(:installments).where(backoffice_status: 2).group("invoices.id").having("SUM(CASE WHEN (liquidation_status = 0) AND (installments.due_date < NOW()) THEN 1 ELSE 0 END) = 0 AND SUM(CASE WHEN liquidation_status > 0 THEN 1 ELSE 0 END) < COUNT(liquidation_status)").order("max(installments.due_date) ASC")}
  scope :opened, -> {joins(:installments).where(backoffice_status: 2).group("invoices.id").having("(SUM(CASE WHEN (liquidation_status = 0) AND (installments.due_date < NOW()) THEN 1 ELSE 0 END) = 0 AND SUM(CASE WHEN liquidation_status > 0 THEN 1 ELSE 0 END) < COUNT(liquidation_status)) OR (SUM(CASE WHEN (liquidation_status = 0) AND (installments.due_date < NOW()) THEN 1 ELSE 0 END) > 0)").order("max(installments.due_date) ASC")}
  scope :paid, -> {joins(:installments).where(backoffice_status: 2).group("invoices.id").having("SUM(CASE liquidation_status WHEN 1 THEN 1 ELSE 0 END) = COUNT(liquidation_status)").order("max(installments.due_date) DESC")}
  scope :rebought, -> {joins(:installments).where(backoffice_status: 2).group("invoices.id").having("SUM(CASE liquidation_status WHEN 2 THEN 1 ELSE 0 END) = COUNT(liquidation_status)").order("max(installments.due_date) DESC")}
  scope :lost, -> {joins(:installments).where(backoffice_status: 2).group("invoices.id").having("SUM(CASE liquidation_status WHEN 3 THEN 1 ELSE 0 END) = COUNT(liquidation_status)").order("max(installments.due_date) DESC")}
  scope :finished, -> {joins(:installments).where(backoffice_status: 2).group("invoices.id").having("(SUM(CASE liquidation_status WHEN 1 THEN 1 ELSE 0 END) = COUNT(liquidation_status)) OR (SUM(CASE liquidation_status WHEN 2 THEN 1 ELSE 0 END) = COUNT(liquidation_status)) OR (SUM(CASE liquidation_status WHEN 3 THEN 1 ELSE 0 END) = COUNT(liquidation_status))").order("max(installments.due_date) DESC")}
